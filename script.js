...
async function checkAndApplySiteStatus() {
  ...
  // 3. Mostrar overlay se necessário
  if (isManual || isWeekend || isOutsideHours) {
    let msg = isManual ? manualMsg
            : isWeekend ? 'Estamos fechados aos fins de semana. Voltamos na segunda! 🍔'
            : 'Estamos fechados agora. Abrimos de segunda a sexta, das 18h às 22h. 🍔';
    showClosedOverlay(msg);
    return false; // site bloqueado
  }
  return true; // site aberto
}

function showClosedOverlay(message) {
  const overlay = document.createElement('div');
  overlay.id = 'site-closed-overlay';
  overlay.innerHTML = `
    <div style="
    ...
    ">
      <div style="max-width:380px;">
        ...
        <h2 style="
          ...
          font-size:36px;color:hsl(38,95%,55%);
          letter-spacing:0.05em;margin-bottom:16px;
        ">Estamos fechados</h2>
        <p style="
          font-family:'Inter',sans-serif;
          color:hsl(40,30%,80%);font-size:15px;line-height:1.6;
        ">${escapeHtml(message)}</p>
        <p style="
          font-family:'Inter',sans-serif;
          color:hsl(40,30%,55%);font-size:13px;margin-top:20px;
        ">Seg–Sex · 18h às 22h</p>
      </div>
    </div>`;
...
document.addEventListener("DOMContentLoaded", async () => {
  // Verifica status do site antes de inicializar tudo
  const isOpen = await checkAndApplySiteStatus();
  if (!isOpen) return; // para aqui se fechado
  initFireParticles();
  initHeroVideo();
  initFeedback();
  renderGrids();
